...
def get_bull_by_register(db: Session, register: str) -> Optional[Bull]:
    ...
    return db.query(Bull).filter(Bull.register == register).first()
    ...
    if search_query:
        ...
        query = query.filter(
            or_(
                ...
                func.lower(Bull.register).like(search_term),
    ...
    for row in results:
        try:
            ...
            bull_data = {
                "id": bull.id,
                "name": bull.name,
                "registration_number": bull.register,
                "lote":bull.lote,
...
def create_bull(db: Session, bull: BullCreate, current_user: User) -> Bull:
    ...
    model_status = None
    if bull.status:
        model_status = bull.status.to_model()
    else:
        # Si no se proporciona un estado, usar el estado activo por defecto
    ...
    db_bull = Bull(
        name=bull.name,
        register=bull.registration_number,
        race_id=bull.race_id,
        sex_id=bull.sex_id,
        status=model_status,
        lote=bull.lote,
...
def update_bull(db: Session, bull_id: int, bull: BullUpdate, current_user: User) -> Optional[Bull]:
    ...
    # Verificar unicidad del número de registro si se está actualizando
    if bull.registration_number and bull.registration_number != db_bull.register:
        existing_bull = get_bull_by_register(db, bull.registration_number)
        if existing_bull:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if "registration_number" in update_data:
        update_data["register"] = update_data.pop("registration_number")
...
def filter_bulls(
    ...
    if register:
        ...
        query = query.filter(func.lower(Bull.register).like(register_term))
